...
import {
	InMemorySyncStorage,
	MAX_TOMBSTONES as IN_MEMORY_MAX_TOMBSTONES,
	TOMBSTONE_PRUNE_BUFFER_SIZE as IN_MEMORY_TOMBSTONE_PRUNE_BUFFER_SIZE,
...
export const MAX_TOMBSTONES = IN_MEMORY_MAX_TOMBSTONES
export const TOMBSTONE_PRUNE_BUFFER_SIZE = IN_MEMORY_TOMBSTONE_PRUNE_BUFFER_SIZE
...
export class TLSyncRoom<R extends UnknownRecord, SessionMeta> {
	...
	/**
	 * @deprecated legacy compatibility surface; prefer `storage.getSnapshot()`.
	 */
	readonly state = {
		get: () => {
			const snapshot = this.storage.getSnapshot?.()
			if (!snapshot) {
				return { documents: {} as Record<string, { state: R; lastChangedClock: number }>, tombstones: {} }
			}
			return {
				documents: Object.fromEntries(
					snapshot.documents.map((doc) => [doc.state.id, { state: doc.state as R, lastChangedClock: doc.lastChangedClock }])
				),
	...
	constructor(opts: {
		...
		if (opts.storage && opts.snapshot) {
			throw new Error('Cannot provide both storage and snapshot options')
		}
		this.storage = opts.storage ?? new InMemorySyncStorage<R>({ snapshot: opts.snapshot })
	...
	handleNewSession(opts: {
		...
		this.sessions.set(sessionId, {
			...
			// v7 protocol does not support string append ops.
			supportsStringAppend: false,
		})
		return this
	}
	...
	private migrateDiffOrRejectSession(
		...
		for (const [id, put] of objectMapEntries(diff.puts)) {
				...
				const txResult = this.storage.transaction(
					(txn) => {
						...
						if (message.diff && !session?.isReadonly) {
							...
							for (const [id, op] of objectMapEntries(message.diff!)) {
								switch (op[0]) {
									case RecordOpType.Put: {
	...
	/**
	 * @deprecated use `storage.getSnapshot()` instead.
	 */
	getSnapshot(): RoomSnapshot {
		if (this.storage.getSnapshot) return this.storage.getSnapshot()
		throw new Error('getSnapshot is not supported for this storage type')
	}

	/**
	 * @deprecated use `storage.getClock()` instead.
	 */
	get clock() {
		return this.lastDocumentClock
	}
	...
	/**
	 * @deprecated use `storage.transaction(...)` instead.
	 */
...
/**
 * @public
 * @deprecated use `TLSyncStorageTransaction` methods instead.
 */
