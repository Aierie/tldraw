import { RecordId, StoreSchema } from '@tldraw/store'
...
let DatabaseSyncCtor: (new (location: string) => DatabaseSync) | null = null
try {
	// Node 22.5+ only. Keep tests loadable in Node 20 by conditionally skipping.
	DatabaseSyncCtor = require('node:sqlite').DatabaseSync as new (location: string) => DatabaseSync
} catch {
	// noop
}
const describeIfNodeSqlite = DatabaseSyncCtor ? describe : describe.skip
...
describeIfNodeSqlite('NodeSqliteSyncWrapper + SQLiteSyncStorage integration', () => {
	let db: DatabaseSync
	...
	describe('hasBeenInitialized', () => {
		...
		it('returns false for uninitialized storage', () => {
			const freshDb = new DatabaseSyncCtor!(':memory:')
