...
const BASE_URL =
	process.env.SIMPLE_SYNC_WORKER_BASE_URL ?? 'http://127.0.0.1:8790'
const CLIENT_BASE_URL =
	process.env.SIMPLE_SYNC_CLIENT_BASE_URL ?? 'http://127.0.0.1:5173'
...
async function startOtelTraceCapture(
	testInfo: TestInfo
): Promise<OtelTraceCapture> {
...
async function readTraceFileContents(traceFile: string) {
	return await readFile(traceFile, 'utf8').catch(
...
function parseTraceEvents(events: unknown): CapturedTraceEvent[] {
	...
	for (const event of events) {
		...
		parsed.push({
			name,
			attributes: parseTraceAttributes(
				(event as { attributes?: unknown }).attributes
			),
		})
	}
...
async function openRoom(page: Page, roomId: string) {
	...
	await page.waitForFunction(
		() => Boolean((window as any).__tldrawEditor),
		null,
		{
			timeout: 15_000,
		}
...
function assertMinimalPushOutcomeSpans(
	spans: Array<{ name: string; attributes: Record<string, unknown> }>
) {
	expect(
		spans.some(
			(span) =>
				span.name === 'tlsync.room.push' || span.name === 'tlsync.room.connect'
		)
	).toBeFalsy()

	for (const span of spans) {
		if (span.name !== 'tlsync.room.push_outcome') continue
		expect(span.name).toBe('tlsync.room.push_outcome')
		const action = span.attributes['tldraw.push_result.action']
		expect(
			action === 'commit' || action === 'discard' || action === 'rebase'
		).toBeTruthy()
	}
}
...
async function waitForConnect(roomId: string) {
	await waitForSnapshot(
		roomId,
		(snapshot) => typeof snapshot.clock === 'number'
	)
}
...
test.describe('simple sync worker e2e', () => {
	test('reset + create/update/delete sequence persists expected tombstone and spans', async ({
		...
		await waitForSnapshot(roomId, (next) =>
			Boolean(getSnapshotRecord(next, shapeId))
		)
		...
		assertMinimalPushOutcomeSpans(spans)
	})

	test('different op ordering keeps final snapshot consistent', async ({
		page,
	}, testInfo) => {
		const roomId = buildRoomId(testInfo)
		...
		await waitForSnapshot(roomId, (next) => {
			return Boolean(
				getSnapshotRecord(next, deletedShapeId) &&
					getSnapshotRecord(next, survivingShapeId)
			)
		})
		...
		await waitForSnapshot(
			roomId,
			(next) => !getSnapshotRecord(next, deletedShapeId)
		)
	...
	test('essential canvas ops write to OTel traces (create/update/group/delete)', async ({
		...
		assertMinimalPushOutcomeSpans(detailedTraceSpans)
	})
	...
	test('essential canvas ops via DOM interaction write to OTel traces (create/move/delete)', async ({
		...
		const afterCreate = await waitForSnapshot(
			roomId,
			(next) => getGeoShapes(next).length === 3
		)
		const sortedGeoShapes = getGeoShapes(afterCreate).sort(
			(a, b) => (a.x ?? 0) - (b.x ?? 0)
		)
		...
		await waitForSnapshot(roomId, (next) => {
			...
			return (
				geos.length === 3 &&
				geos.some((shape) => typeof shape.y === 'number' && shape.y > 42)
			)
		})
		...
		const afterDelete = await waitForSnapshot(roomId, (next) => {
			return (
				getGeoShapes(next).length === 0 &&
				Object.keys(next.tombstones ?? {}).length >= 3
			)
		})

		expect(
			Object.keys(afterDelete.tombstones ?? {}).length
	...
	test('reload keeps grouped + ungrouped mixed deletion state and minimal OTel outcomes', async ({
		page,
	}, testInfo) => {
		const collectorReachable = await isCollectorReachable()
		expect(
		...
		await waitForSnapshot(roomId, (next) => {
			...
			const groupedSiblingSurvivor = getSnapshotRecord(
				next,
				groupedSiblingSurvivorId
			)
		...
		await runCanvasOps(page, [
			{ kind: 'delete', ids: [groupedDeletedId, ungroupedDeletedId] },
		])
		...
		expect(
			getSnapshotRecord(beforeReloadSnapshot, groupedSurvivorId)?.parentId
		).toBe(groupId)
		expect(
			getSnapshotRecord(beforeReloadSnapshot, groupedSiblingSurvivorId)
				?.parentId
		).toBe(groupId)
		expect(
			getSnapshotRecord(beforeReloadSnapshot, ungroupedSurvivorId)?.parentId
		).not.toBe(groupId)
		...
		const afterReloadSnapshot = await waitForSnapshot(
			roomId,
			hasExpectedState,
			reloadTimeoutMs
		)
		expect(
			getSnapshotRecord(afterReloadSnapshot, groupedSurvivorId)?.parentId
		).toBe(groupId)
		expect(
			getSnapshotRecord(afterReloadSnapshot, groupedSiblingSurvivorId)?.parentId
		).toBe(groupId)
		expect(
			getSnapshotRecord(afterReloadSnapshot, ungroupedSurvivorId)?.parentId
		).not.toBe(groupId)
	...
	test('reset wipes an already-dirty room back to baseline snapshot', async ({
		...
		await waitForSnapshot(roomId, (snapshot) =>
			Boolean(getSnapshotRecord(snapshot, shapeId))
		)
		await resetRoom(roomId)
